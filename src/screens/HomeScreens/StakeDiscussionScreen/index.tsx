...
const StakeDiscussionScreen = () => {
  ...
  useEffect(() => {
    ...
    // Listen for new messages to trigger immediate fetch
    socketRef.current.on('receiveMessage', (newMessage: any) => {
      // Check if the message belongs to this chat
      if (
        newMessage.betId === betId &&
        (newMessage.senderId === friendId || newMessage.receiverId === friendId)
      ) {
        // Trigger immediate fetch to get the latest messages from the API
        fetchMessages();
      }
    });

    // Clean up socket and polling interval on unmount
    return () => {
  ...
  }, [betId, friendId, token]); // Dependencies for useEffect
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // Trigger immediate fetch to update messages from API
        fetchMessages();
      }
    } catch (err) {
        ...
        {isMe ? (
          ...
          <>
            ...
            <View style={tw`max-w-[70%] items-end`}>
              <Text style={tw`font-bold text-xs mb-1`}>{friendName}</Text>
              <Text style={tw`text-black text-xs font-regular`}>
                {item.message}
              </Text>
            </View>
